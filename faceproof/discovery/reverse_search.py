...
from faceproof.discovery.retrieval import REQUEST_TIMEOUT
...
def reverse_search(
    ...
    if not api_key:
        raise RuntimeError("SERPAPI_API_KEY is not set (copy .env.example to .env)")
    ...
    try:
        ...
        raise RuntimeError(_failure_reason(error)) from None

    payload = response.json()
    if payload.get("error"):
        raise RuntimeError(f"reverse image search error: {payload['error']}")
